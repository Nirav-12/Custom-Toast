...
function FileUpload({navigation}) {
  ...
  const keyWordCompile = () => {
    ...
    for (let i = 0; i < mainObj.length; i++) {
      if (!obj[mainObj[i].Symbol]) {
        let valForSymbol = {
          buyValue: mainObj[i]['Buy Value'],
          Quantity: mainObj[i]['Quantity'],
          SellValue: mainObj[i]['Sell Value'],
        };
        obj[mainObj[i].Symbol] = valForSymbol;
      } else {
        obj[mainObj[i].Symbol]['buyValue'] =
          obj[mainObj[i].Symbol]['buyValue'] + mainObj[i]['Buy Value'];
        obj[mainObj[i].Symbol]['Quantity'] =
          obj[mainObj[i].Symbol]['Quantity'] + mainObj[i]['Quantity'];
        obj[mainObj[i].Symbol]['SellValue'] =
          obj[mainObj[i].Symbol]['SellValue'] + mainObj[i]['Sell Value'];
      }
    }
  ...
  return (
    <View style={styles.container}>
      <NaviButton title="Get data" onPress={() => getData()} />

      <NaviButton title="Final object" onPress={() => finalObj()} />

      <NaviButton title="keyword compile" onPress={() => keyWordCompile()} />
    </View>
  );
}
